...
### 2026-08-12 · Phase 2 공급업체 변경 승인 — ~~Open-Meteo BOM ACCESS-G~~ Open-Meteo Best Match

- ~~Phase 2는 WeatherAPI.com Free-only·`WEATHERAPI_KEY` Secret·월 10,000회·현재 60분/예보 24시간 캐시를 사용한다.~~<br>~~사용자는 최신 공식 비교 뒤 Open-Meteo의 BOM ACCESS-G를 승인했다.~~<br>실제 BOM open-data 일시 중단을 확인한 뒤 같은 Open-Meteo의 공식 권장 Best Match를 비공개·비상업 커플 여행 앱의 실제 날씨 모델 선택으로 사용한다. WeatherAPI.com 계정·Secret 발급과 실제 호출은 더 이상 Phase 2 완료 조건이 아니다.
- API key·결제 카드·자동 과금 없이 사용할 수 있고 현재 제품의 비공개·비상업 사용 조건과 Open-Meteo Free 약관이 맞는다. Best Match는 지역별 가용 모델을 자동 선택하며 BOM이 다시 제공되면 후보로 사용할 수 있다. Open-Meteo Free의 공급업체 한도(월 300,000회)와 별개로, 제품 자체 월 **10,000 provider request** 원자 차단은 비용·장애 보호 기준으로 유지한다.
- Open-Meteo는 좌표 기반 API이므로 cold/full refresh 때 여행 목적지 문자열을 공식 geocoding endpoint로 해석한 뒤 Forecast Best Match endpoint를 호출한다. 같은 목적지의 forecast 캐시가 남아 current만 갱신할 때는 저장된 좌표를 재사용해 geocoding 호출을 만들지 않는다. full refresh의 geocoding+forecast 두 호출과 current-only 갱신 한 호출을 각각 제품 한도에 계산한다.
- D1 `0023`의 current·forecast 분리 구조와 현재 60분/24시간 제품 캐시 정책은 유지한다. raw provider body는 저장하지 않으며, 목적지 query와 저장 query가 일치할 때만 캐시를 재사용한다. Open-Meteo Best Match의 현재·일별 예보 정규화, WMO 코드 한국어 표시, `Open-Meteo · Best Match` 출처와 일반 참고용 면책을 구현·테스트한다.
