...
>
> 기준 제품 커밋: `8cf0d978f87cbda785f41afce4ea3c7f6470114c` (2026-08-11 Phase 1 1차 묶음·관리자 Access 복구 배포 기준)
...
| Phase 2 실제 날씨 | 로컬 구현·자동 검증 완료, 운영 연결 대기 | ~~WeatherAPI.com Free-only·월 10,000회·현재 60분/예보 24시간·출처·면책을 확정했고 Worker·D1 0023 초안·Today 상태 UI를 구현~~<br>2026-08-12 Open-Meteo BOM ACCESS-G 전환을 로컬 구현했다. 제품 월 10,000 provider request·현재 60분/예보 24시간·출처·면책과 Worker·D1 0023·Today 상태 UI를 유지한다. | 운영 D1 `0023` 적용, 실제 Open-Meteo 호출 1회, 사용자·관리자 배포와 최종 통합 QA |
...
**2026-08-11 1차 Phase 1 통합 묶음 배포 기록:** 제품 커밋 `04b391722a5cf99750565ccc6f8565094fafdd50`과 관리자 Access 복구 커밋 `8cf0d978f87cbda785f41afce4ea3c7f6470114c`를 `main`에 push했다. 최종 재검증은 frontend 68파일 485건, Worker 10파일 95건, `typecheck`, 전체 lint, production build, Playwright 46건, `git diff --check`를 통과했다. 사용자 Worker version `2f2e4799-6b65-4e9c-8bbe-e679377698b3`, 관리자 Worker version `c35a6d6b-efb3-4aaf-9fe7-cf10d3aa8c55`를 배포했다. 사용자 root·health·manifest·service worker·bundle은 HTTP 200이고 운영 bundle `/assets/index-COr6lnbW.js`와 service worker의 로컬·원격 SHA-256이 일치했다. 관리자 root·health는 설정된 Access host로 HTTP 302였다. migration·Secret·Places 800회 소진·Android 카메라 JPEG 입력은 실행하지 않았다.
...
### 2026-08-12 · Phase 2 공급업체 변경 승인 — Open-Meteo BOM ACCESS-G

- ~~Phase 2는 WeatherAPI.com Free-only·`WEATHERAPI_KEY` Secret·월 10,000회·현재 60분/예보 24시간 캐시를 사용한다.~~<br>사용자는 최신 공식 비교 뒤 **Open-Meteo의 BOM ACCESS-G**를 비공개·비상업 커플 여행 앱의 실제 날씨 공급업체로 승인했다. WeatherAPI.com 계정·Secret 발급과 실제 호출은 더 이상 Phase 2 완료 조건이 아니다.
- 이유는 호주 기상청 BOM 모델을 명시적으로 선택할 수 있고, API key·결제 카드·자동 과금 없이 사용할 수 있으며, 현재 제품의 비공개·비상업 사용 조건과 Open-Meteo Free 약관이 맞기 때문이다. Open-Meteo Free의 공급업체 한도(월 300,000회)와 별개로, 제품 자체 월 **10,000 provider request** 원자 차단은 비용·장애 보호 기준으로 유지한다.
- Open-Meteo는 좌표 기반 API이므로 cold/full refresh 때 여행 목적지 문자열을 공식 geocoding endpoint로 해석한 뒤 BOM endpoint를 호출한다. 같은 목적지의 forecast 캐시가 남아 current만 갱신할 때는 저장된 좌표를 재사용해 geocoding 호출을 만들지 않는다. full refresh의 geocoding+forecast 두 호출과 current-only 갱신 한 호출을 각각 제품 한도에 계산한다.
- D1 `0023`의 current·forecast 분리 구조와 현재 60분/24시간 제품 캐시 정책은 유지한다. raw provider body는 저장하지 않으며, 목적지 query와 저장 query가 일치할 때만 캐시를 재사용한다. Open-Meteo/BOM의 현재·일별 예보 정규화, WMO 코드 한국어 표시, `Open-Meteo · BOM ACCESS-G` 출처와 일반 참고용 면책을 구현·테스트한다.
- 이 승인에는 로컬 계획·코드·대상 자동 검증만 포함한다. 새 코드의 commit·push, 운영 D1 `0023` 적용, 실제 Open-Meteo 호출, 두 Worker 배포, 운영/실기기 QA와 V1 tag는 새 공급업체 기준으로 별도 재승인한다.
