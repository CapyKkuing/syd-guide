...
| GAP-34 | 동일 세션에서 노출된 Places SKU 누적 사용량이 응답 순서 때문에 감소하지 않음 | 2026-08-09 로컬 통합 구현·자동 검증 완료. `PlacesApi`가 같은 세션에서 SKU별 최대 노출값을 기억해 새·캐시·늦은 응답을 결합한다. 운영 재검증은 미배포 상태라 대기한다. | SKU별 최대 노출값을 유지해 늦거나 캐시된 응답의 작은 값은 표시를 낮추지 않고 실제 증가 응답은 반영한다. | Phase 1B 보정 |
