...
|---|---|---|---|---|
| GAP-01 | 장소 탭 진입 즉시 저장 맛집·카페와 최신 추천을 통합 표시 | ~~로컬 구현·화면 검증~~<br>로컬 구현·화면 검증, 운영 정적 bundle 반영 확인, 운영 실데이터 행동 미검증 | 운영 인증 세션에서 실제 중복 통합·필터·정렬·길찾기 확인 | Phase 1B |
...
| GAP-03 | 장소 상세·투표·지도 관련 구형 UI를 공통 디자인으로 통일 | ~~로컬 구현·화면 검증~~<br>~~부분 구현. 장소 허브 목록·상세·투표·편집은 보정됐고 지도 보기 헤더·검색·필터·장소 목록은 구형 UI가 남음~~<br>2026-08-03 지도 보기 헤더·검색·필터·결과·장소 목록까지 공통 UI로 보정하고 로컬 화면 검증 완료 | ~~지도 보기까지 공통 디자인으로 보정하고 실제 설치폰에서 상세·투표·편집·지도 터치와 넘침 최종 확인~~<br>실제 설치폰에서 상세·투표·편집·지도 터치와 넘침 최종 확인 | Phase 1B |
...
| GAP-06 | 3명 이상 정산의 모든 송금 표시와 개별 완료 | ~~로컬 구현·자동 검증~~<br>로컬 구현·자동 검증·운영 migration 적용 | 복수 송금이 운영 데이터에서 모두 보이고 한 송금 완료가 다른 송금을 완료시키지 않음 | Phase 1C |
| GAP-07 | 예약의 체크인 전·완료 상태와 탑승권 보관 | ~~로컬 구현·자동 검증~~<br>로컬 구현·자동 검증·운영 migration 적용 | 운영에서 예약 상태 저장·수정과 관련 파일 미리보기 확인 | Phase 1C |
| GAP-18 | 예약증·항공권 이미지/PDF OCR로 예약 정보 초안 자동 채우기 | ~~미구현~~<br>2026-08-08 로컬 구현·자동 검증. 이미지·PDF를 사용자가 명시 실행하면 Worker가 Google Vision 공통 초안으로 변환하고 저장 전 편집할 수 있다. 월 800페이지 차단·미설정·한도 초과 때 수동 입력을 유지한다. 운영 Vision API·Secret·`0022` migration·실제 호출은 미승인·미적용 | OCR 결과는 저장 전 사용자가 검토·수정하고, 무료 하드 리밋 초과 시 수동 입력이 항상 가능 | Phase 1C 보완 |
| GAP-08 | 대표사진 미리보기 자르기·밝기, Drive 원본 비변경 | ~~로컬 구현·자동·390px 화면 검증~~<br>로컬 구현·자동·390px 화면 검증. 편집 저장은 Drive 원본 재업로드 없이 preview metadata만 변경 | ~~편집값은 미리보기에만 적용되고 원본 checksum·파일이 유지됨~~<br>실제 Drive 원본 checksum·파일 보존은 운영 연결 뒤 별도 확인 | Phase 1D |
...
| GAP-22 | 빈 체크리스트에 승인된 기본 항목을 한 번만 생성 | ~~로컬 migration·자동·화면 검증~~<br>운영 legacy 여행에서 `완료 0 / 전체 0`, `필수 미등록`, `예약 미등록` 확인. 운영 D1에 `0019_default_checklist.sql` 적용 증거가 없어 기본 항목이 생성되지 않음 | 신규·기존 빈 여행에 한 번 생성되고 삭제 뒤 재생성되지 않으며 자유 편집 가능. `0019` 운영 migration 적용 뒤 기존 빈 여행에서 1회 생성·화면 확인 | Phase 1C 보정 |
| GAP-23 | 관리자 `Failed to fetch`를 Access 재로그인 복구 흐름으로 통일 | 로컬 자동 검증 | 참여자·관리·여행 snapshot 모두 재로그인 버튼과 원래 경로 복귀를 제공 | Phase 1 보정 |
| GAP-24 | 장소 필터·정렬만 바꿀 때 새 사진 공급업체 호출이 반복되지 않음 | ~~2026-08-03 운영 QA에서 불일치 발견~~<br>~~근본 원인 확인, 사진 자동 표시 정책 선택 대기~~<br>C안 승인, 로컬 구현·자동 검증·브라우저 화면 확인 | ~~이미 받은 사진 URL을 세션 안에서 재사용하고 필터·정렬만으로 Places 사용량이 증가하지 않음~~<br>~~동일 사진은 현재도 세션 중복 제거된다. 처음 노출되는 서로 다른 장소 사진을 자동 호출할지, 상세 열기 때만 호출할지 사용자 승인 후 완료 조건을 확정~~<br>여행별 앱 세션의 첫 6장만 자동 호출하고 이후 새 카드는 상세 보기를 눌렀을 때만 1회 호출. 운영 배포 뒤 필터·정렬은 +0, 상세 열기는 해당 사진만 +1인지 확인 | Phase 1B 보정 |
...
현재는 약 **40%**로 본다. 기능 코드 개수보다 운영 증거를 우선한 수치다. Phase 0의 코드·배포는 완료됐지만 최신 배포본의 실기기 편집창 확인 1건을 아직 운영 완료 증거로 남겨두었다. 대표사진 편집은 로컬 구현·자동·390px 화면 검증까지 끝났지만 D1 migration 적용·운영 원본 보존 확인 전에는 진행률을 올리지 않는다.
...
| ~~Android·iPhone PWA 오프라인~~<br>Android·iPhone 온라인 전용 PWA·동기화 | 20 | ~~5~~<br>3 | 두 기기에서 설치·오프라인 차단·온라인 복귀·서버 직접 저장·5초 이내 다중 기기 갱신 통과 |
